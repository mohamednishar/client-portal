...
export class ClientService {
  ...
  public loadClientConfig(clientId: string): Observable<ClientConfig> {
    ...
    return this.http.get<ClientConfig>(jsonPath).pipe(
      tap((config) => {
        ...
        // Apply dynamic client theme
        if (config.theme) {
          this.themeService.applyTheme(config.theme);
        }

        // Persist active clientId in localStorage
        this.storageService.setItem(STORAGE_KEYS.CURRENT_CLIENT_ID, config.clientId);
      }),
      catchError((error) => {
        this.isLoadingSignal.set(false);
  ...
  public getClientId(): string | null {
    return this.currentClientSignal()?.clientId || this.storageService.getItem(STORAGE_KEYS.CURRENT_CLIENT_ID);
  }
